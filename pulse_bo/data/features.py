"""
Functions for engineering feature inputs, loading the dataset, and scaling.
"""
...
def extract_features(file: str):
    """Loads every valid sheet and returns engineered features, targets, and the feasibility mask."""
    ...
    for _sheet, df in all_data.items():
        ...
        if not all(c in df.columns for c in needed):
            continue
        # Stable row order so CV folds do not depend on workbook order
