...
@jit(nopython=True)
def calculate_cross_term_ni_o(coords_Co, coords_O, lbox, nbins, bin_start, bin_end):
    n1 = coords_Co.shape[0]  # number of Co atoms
    ...
    for i in range(n1):
        ...
        x_dist = np.abs(coords_Co[i, 0] - coords_O[:, 0])
        y_dist = np.abs(coords_Co[i, 1] - coords_O[:, 1])
        z_dist = np.abs(coords_Co[i, 2] - coords_O[:, 2])
        # PBC correction
...
frame_idx = 0
Co_atoms = u.select_atoms('type 3')
...
for i, ts in enumerate(u.trajectory):
    ...
    pos_Co = Co_atoms.positions
    pos_o = O_atoms.positions

    r, gr_ni_o_total[frame_idx, :] = calculate_cross_term_ni_o(pos_Co, pos_o, lbox, nbins, bin_start, bin_end)
...
print(f"RDF saved to Co-O.txt")

# ===================== COORDINATION NUMBER (Li–O in EC) ===================== #
rho_CoO = O_atoms.n_atoms / (lbox[0] * lbox[1] * lbox[2])
print('rho_CoO =', rho_CoO)

r_min = 2.8

CN_blocks = np.zeros(nblocks)
for block in range(nblocks):
    CN_blocks[block] = coordination_number(r_block, gr_ni_o_block_avg[block, :], rho_CoO, r_min)

CN_mean = np.mean(CN_blocks)
CN_std  = np.std(CN_blocks, ddof=1)      
CN_ste  = CN_std / np.sqrt(nblocks)
print("CN_CoO =", CN_mean, "+/-", CN_ste)
